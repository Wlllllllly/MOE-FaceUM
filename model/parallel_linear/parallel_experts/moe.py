...
class TaskMoE(MoE):
    ...
    def __init__(self,  input_size, head_size, num_experts, k, w_MI=0, limit_k=0, w_topk_loss=0.0, task_num=2, noisy_gating=True, gating_activation=None, **kwargs):
        ...
        self.tot_expert =[num_experts if noisy_gating else num_experts for i in range(task_num)] 
        self.top_k_gates_task = [torch.Size([])] * 6#torch.zeros(task_num, self.num_experts).cuda()
        self.top_k_indices_task = [torch.Size([])] * 6#torch.zeros(task_num, self.num_experts).cuda()
        self.f_gate = nn.ModuleList([nn.Sequential(
                                        # nn.Linear(input_size, input_size),
                                        # gating_activation,
                                        nn.Linear(input_size,
                                                  num_experts if noisy_gating else num_experts,
                                                  bias=False)
                                    ) for i in range(task_num)])
        self.expert_biases = [nn.Parameter(torch.zeros(num_experts if noisy_gating else num_experts).cuda()) for i in range(task_num)]
    ...
    def top_k_gating(self, x, task_bh, skip_mask=None, sample_topk=0, noise_epsilon=1e-2):
        ...
        """Noisy top-k gating.
          ...
          Returns:
            ...
            load: a Tensor with shape [num_experts]
        """
        clean_logits_bias = self.f_gate[task_bh](x) + self.expert_biases[task_bh]
        ...
        self.top_k_gates_task[task_bh] = top_k_gates
        self.top_k_indices_task[task_bh]  = top_k_indices
    ...
    def forward(self, x, task_bh, skip_mask=None, sample_topk=0, multiply_by_gates=True):
        # y_ = self.forward_(x, skip_mask, sample_topk, multiply_by_gates)
        # raise Exception(f"x:{x.shape}")
        x = x.permute(1, 0, 2)
        bsz, length, emb_size = x.size()
        # cls_token = x[0:1, : , :]
        # patch_tokens = x[1:,:, :] 
        cls_token = x[:, 0:1, :]          # (B, 1, D)
        patch_tokens = x[:, 1:, :]        # (B, L-1, D)

        patch_tokens = patch_tokens.reshape(-1, emb_size)
        if skip_mask is not None:
            skip_mask = skip_mask.view(-1, 1)
        loss = self.top_k_gating(patch_tokens, task_bh, skip_mask,  sample_topk=sample_topk)

        expert_inputs = patch_tokens[self.batch_index]
        h = self.experts(expert_inputs, self.expert_size)
        h = self.activation(h)
        expert_outputs = self.output_experts(h, self.expert_size)
        ...
        # zeros = torch.zeros((bsz * length, self.input_size), 
        #     dtype=expert_outputs.dtype, device=expert_outputs.device)
        patch_output  = zeros.index_add(0, self.batch_index, expert_outputs)
        # y = patch_output.view( length, bsz, self.input_size)
        patch_output = patch_output.view( bsz, length-1,  self.input_size)
        cls_output = self.shared_mlp(cls_token)
        y = torch.cat([cls_output, patch_output], dim=1)
        y = y.permute(1, 0, 2)
        # assert torch.allclose(y, y_)
        return y#, 0#loss
    ...
    def map(self, x, task_bh, skip_mask=None, sample_topk=0):
        ...
        zeros = torch.zeros((bsz * length * self.k, self.head_size), 
            dtype=expert_outputs.dtype, device=expert_outputs.device)
        # raise Exception(f"zeros:{zeros.shape},self.index_sorted_experts:{self.index_sorted_experts.shape}, expert_outputs:{expert_outputs.shape}")
...
class TaskMoE_Face(MoE):
    ...
    def __init__(self,  input_size, head_size, num_experts, k, w_MI=0, limit_k=0, w_topk_loss=0.0, task_num=2, noisy_gating=True, gating_activation=None, **kwargs):
        ...
        super(TaskMoE_Face, self).__init__(input_size, head_size, num_experts, k, noisy_gating=noisy_gating, gating_activation=gating_activation, **kwargs)
        ...
        self.expert_biases= [nn.Parameter(torch.zeros(2 * num_experts if noisy_gating else num_experts).cuda()) for i in range(task_num)]
        # self.expert_biases_nonroi = [nn.Parameter(torch.zeros(2 * num_experts if noisy_gating else num_experts).cuda()) for i in range(task_num)]
    ...
    def top_k_gating(self, x, task_bh, skip_mask=None, sample_topk=0, noise_epsilon=1e-2):
        ...
        """Noisy top-k gating.
          ...
          Returns:
            ...
            load: a Tensor with shape [num_experts]
        """
        clean_logits = self.f_gate[task_bh](x)+self.expert_biases[task_bh]
    ...
    def update_bias_loss_free(self, b: torch.Tensor, rho: torch.Tensor, alpha: float, k: int):
        """
        更新 gating bias b，依据无损负载均衡机制（公式 8）

        Args:
            b: 当前 gating bias，shape = [num_experts]
            rho: 当前 gating logits，shape = [num_experts]
            alpha: 学习率（控制更新步长）
            k: 允许激活的最大 expert 数量（top-k 限制）

        Returns:
            更新后的 b
        """

        # Step 1: 计算 f_i = 1 if rho_i + b_i > 0 else 0
        f = ((rho + b) > 0).float()  # [num_experts]

        # Step 2: 归一化得到 F = f / |f|
        f_sum = f.sum() + 1e-6  # 避免除以 0
        F = f / f_sum

        # Step 3: 构造均匀分布 Q
        n = f.numel()
        Q = torch.full_like(F, 1.0 / n)  # [1/n, 1/n, ..., 1/n]

        # Step 4: sign(F - Q)
        sign_diff = torch.sign(F - Q)

        # Step 5: sign(max(|F| - k, 0))
        overuse = torch.sign(torch.clamp(f_sum - k, min=0.0))

        # Step 6: b ← b - α * [sign(F - Q) + sign(max(|F| - k, 0))]
        b = b - alpha * (sign_diff + overuse)

        return b

    def top_k_gating_roi(self, x, task_bh, skip_mask=None, sample_topk=0, noise_epsilon=1e-2):
        ...
        """Noisy top-k gating.
          ...
          Returns:
            ...
            load: a Tensor with shape [num_experts]
        """
        clean_logits = self.f_gate[task_bh](x)+self.expert_biases_roi[task_bh]
        ...
        for i in range(B):
            mask = probs[i] > 0
            indices = torch.nonzero(mask, as_tuple=False).squeeze(1)  # shape: [Ki]
            values = probs[i, indices]  # shape: [Ki]
            dynamic_k_indices.append(indices)
            dynamic_k_gates.append(values)
            self.k=len(indices)
            ic(indices)
    

        # Padding to max_k for batching
        max_k = max(len(x) for x in dynamic_k_indices)
        padded_indices = []
        padded_gates = []

        for indices, gates in zip(dynamic_k_indices, dynamic_k_gates):
            pad_len = max_k - len(indices)
            if pad_len > 0:
                indices = F.pad(indices, (0, pad_len), value=0)
                gates = F.pad(gates, (0, pad_len), value=0.0)
            padded_indices.append(indices)
            padded_gates.append(gates)


        top_k_indices = torch.stack(padded_indices, dim=0)  # [B, max_k]
        top_k_gates = torch.stack(padded_gates, dim=0)      # [B, max_k]

        self.expert_biases_roi[task_bh] = self.update_bias_loss_free(b=self.expert_biases_roi[task_bh], rho=probs.mean(dim=0) , alpha=0.001, k=5)
    ...
    def forward(self, x, task_bh, skip_mask=None, sample_topk=0, multiply_by_gates=True,patch_indices_all = None, split_point_all = None):
        ...
        # Helper function to create masked input
        ...
        def get_roi_MOE(input_tensor,skip_mask=None,multiply_by_gates=None, is_roi=None):
            ...
            if skip_mask is not None:
                skip_mask = skip_mask.view(-1, 1)
            # if is_roi:
            #     loss = self.top_k_gating_roi(input_tensor, task_bh, skip_mask,  sample_topk=sample_topk)
            # else:
            loss = self.top_k_gating(input_tensor, task_bh, skip_mask,  sample_topk=sample_topk)
        ...
        for i in range(bsz):
            ...
            if len(roi_indices) > 0:
                ...
                # ic(ri)
                # ic(full_output.shape)    
                # ic(core_out_roi.shape)               
        ...
        # full_output = full_output.permute(1, 0, 2)
        cls_output = self.shared_mlp(cls_token)
...
class TaskMoE_Face_twoR(MoE):
    ...
    def __init__(self,  input_size, head_size, num_experts, k, w_MI=0, limit_k=0, w_topk_loss=0.0, task_num=2, noisy_gating=False, gating_activation=None, **kwargs):
        ...
        super(TaskMoE_Face_twoR, self).__init__(input_size, head_size, num_experts, k, noisy_gating=noisy_gating, gating_activation=gating_activation, **kwargs)
        

                # 假设模型支持 num_tasks 个任务，每层有 num_experts 个 expert
        self.record_expert_usage = True  # 是否开启统计
        ...
        self.tot_expert =[2 *num_experts if noisy_gating else num_experts for i in range(task_num)] 
        self.top_k_gates_task = [torch.Size([])] * 2 #torch.zeros(task_num, self.num_experts).cuda()
        self.top_k_indices_task = [torch.Size([])] * 2 #torch.zeros(task_num, self.num_experts).cuda()
        self.f_gate = nn.ModuleList([nn.Sequential(
                                        # nn.Linear(input_size, input_size),
                                        # gating_activation,
                                        nn.Linear(input_size,
        ...
        self.expert_biases= [nn.Parameter(torch.zeros(2 * num_experts if noisy_gating else num_experts).cuda()) for i in range(2)]
        # self.expert_biases_nonroi = [nn.Parameter(torch.zeros(2 * num_experts if noisy_gating else num_experts).cuda()) for i in range(task_num)]
    ...
    def top_k_gating(self, x, task_bh, skip_mask=None, sample_topk=0, noise_epsilon=1e-2, is_roi=None):
        ...
        if is_roi:
            roi_index=0
            clean_logits = self.f_gate[roi_index](x)
            clean_logits_bias = self.f_gate[roi_index](x)+self.expert_biases[roi_index]
        else:
            roi_index=1
            clean_logits = self.f_gate[roi_index](x)
            clean_logits_bias = self.f_gate[roi_index](x)+self.expert_biases[roi_index]
        ...
        self.top_k_gates_task[roi_index] = top_k_gates
        self.top_k_indices_task[roi_index]  = top_k_indices
    ...
    def forward(self, x, task_bh, skip_mask=None, sample_topk=0, multiply_by_gates=True,patch_indices_all = None, split_point_all = None):
        ...
        def get_roi_MOE(input_tensor,skip_mask=None,multiply_by_gates=None, is_roi=None):
            ...
            #     loss = self.top_k_gating_roi(input_tensor, task_bh, skip_mask,  sample_topk=sample_topk)
            # else:
            loss = self.top_k_gating(input_tensor, task_bh, skip_mask,  sample_topk=sample_topk, is_roi=is_roi)
...
class TaskMoE_Face_sixR(MoE):
    ...
    def top_k_gating_noise(self, x, task_bh, skip_mask=None, sample_topk=0, noise_epsilon=1e-2, is_roi=None):
        ...
        if self.noisy_gating and self.training:
            ...
            clean_logits_bias, raw_noise_stddev_bias = clean_logits.chunk(2, dim=-1)
            noise_stddev_bias = F.softplus(raw_noise_stddev_bias) + noise_epsilon
            eps_bias = torch.randn_like(clean_logits_bias)
            noisy_logits_bias = clean_logits_bias + eps * noise_stddev_bias
            logits_bias = noisy_logits_bias

        elif self.noisy_gating:
            logits, _ = clean_logits.chunk(2, dim=-1)
            logits_bias, _ = clean_logits_bias.chunk(2, dim=-1)
        else:
            logits = clean_logits
            logits_bias = clean_logits_bias
        ...
        if self.training and (sample_topk > 0):
            ...
            _, top_km1_indices = probs_bias.topk(self.k - sample_topk, dim=1)
            # ic(top_km1_indices.shape)
            masked_probs = probs_bias + 1e-6
            masked_probs[torch.arange(probs_bias.size(0)).unsqueeze(
                1), top_km1_indices] = 0
            k_indices = torch.multinomial(masked_probs, sample_topk)
            top_k_indices = torch.cat([top_km1_indices, k_indices], dim=-1)
            top_k_gates = torch.gather(probs_bias, 1, top_k_indices)
        else:
            top_k_gates, top_k_indices = probs_bias.topk(self.k, dim=1)
        ...
        if not self.training and self.record_expert_usage:
            # top_k_indices: shape = [B, k]
            with torch.no_grad():
                expert_counts = torch.bincount(top_k_indices.view(-1), minlength=self.num_experts)
                self.expert_usage_counter[task_bh] += expert_counts.to(self.expert_usage_counter.device)
                # print(f"task_bh:{task_bh}")
    ...
    def top_k_gating_roi2_noise(self, x, task_bh, skip_mask=None, sample_topk=0, noise_epsilon=1e-2, is_roi=None, treshold=0.1):
        ...
        selected_token_sum = selected_token.sum(1, keepdim=True)
        selected_token_mean = np.mean(selected_token_sum.detach().cpu().numpy())
        self.k = min(int(round(selected_token_mean))+2, 6)
        _, top_km1_indices = probs_bias.topk(self.k - sample_topk, dim=1)
        # ic(top_km1_indices.shape)
        masked_probs = probs_bias + 1e-6
        masked_probs[torch.arange(probs_bias.size(0)).unsqueeze(
...
class TaskMoE_Face_sixR_copy(MoE):
    ...
    def top_k_gating_roi2(self, x, task_bh, skip_mask=None, sample_topk=0, noise_epsilon=1e-2, is_roi=None, treshold=0.5):
        ...
        if is_roi:
            ...
            self.k = max(2,int(round(selected_token_mean)))
        else:
            self.k = 2
