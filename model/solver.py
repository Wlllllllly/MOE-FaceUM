...
class Solver(object):
    ...
    def tb_logging(self, output, loss):
        if self.C.rank == 0:
            ...
            # tmp.auxloss.update(output['auxloss'].item())
            ...
            # self.tb_logger.add_scalar('auxloss',output['auxloss'].item(), tmp.current_step)
    ...
    def evaluate(self):
        ...
        if self.cfg.get('evaluate_interval', -1) > 0 and ((self.tmp.current_step + 1) % self.cfg.evaluate_interval == 0 or self.tmp.current_step + 1 == self.cfg.max_iter) and self.C.rank == 0:
            ...
            # self.model.set_mode_to_evaluate()
            ...
            # self.model.set_mode_to_train()


    def test(self):
        """
        Performs the testing procedure for the model. The steps include:
        1. Initialization and model creation
        2. Loading model state and step information
        3. Creating evaluators for each task
        4. Setting model to evaluation mode
        5. Running evaluation for each task using the corresponding evaluator
        
        Note: The model is expected to be in a distributed module format (using .module).
        """
        printlog(f"Initialize!")
