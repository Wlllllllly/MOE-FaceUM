...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor,**kwargs):
        ...
        if "is_dynamic" in kwargs:
            norm_x=self.ln_2(x)
            mlp_x, moe_loss, _ = self.mlp(norm_x)
            x = x + self.drop_path(mlp_x)
            return x,moe_loss
        else:
            if "patch_indices_all" in kwargs:
                patch_indices_all=kwargs["patch_indices_all"]
                split_point_all=kwargs["split_point_all"]
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_all=False,**kwargs):
        if "task_j" in kwargs:
            all_x = []
            for i, blk in enumerate(self.resblocks):
                if self.training and self.use_checkpoint:
                    x = checkpoint.checkpoint(blk, x,task_j=task_j)
                else:
            ...
            if return_all:
                ...
                return x
        elif "is_dynamic" in kwargs:
            all_x = []
            for i, blk in enumerate(self.resblocks):
                if self.training and self.use_checkpoint:
                    x,l_aux = checkpoint.checkpoint(blk, x,is_dynamic=is_dynamic)
                else:
                    x,l_aux = blk(x,is_dynamic=is_dynamic)
                if return_all:
                    all_x.append(x)
            if return_all:
                return all_x,l_aux
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if"task_j" in kwargs:
            x = self.transformer(x, task_j=task_j)
        elif "is_dynamic" in kwargs:
            x, l_aux = self.transformer(x, is_dynamic=is_dynamic)
        else:
            x = self.transformer(x, patch_indices_all = patch_indices_all, split_point_all = split_point_all)
...
class FaRLVisualFeatures_TaskMOE(nn.Module):
    ...
    def __init__(self, model_type: str,
        ...
        self.img_types=['fr','age','attribute','expression', 'fp','landmark']
        # self.task_embedding = nn.Parameter(torch.randn(1, len(self.img_types), 768))
    ...
    def forward(self, image: torch.Tensor,task_j = None) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
        x = image
        results, combined_all, split_point_all = Retina_detect(x)##
        patch_indices_all=torch.tensor(combined_all).cuda()
        split_point_all=torch.tensor(split_point_all).cuda()
